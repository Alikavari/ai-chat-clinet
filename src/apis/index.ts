...
export async function getClaimSignatureFromPastAPI(claimer: string) {
  const response = await axiosInstance.get('/reward-server/rewards?claimer=' + claimer, {
    headers: {
      'Content-Type': 'application/json'
    }
  });
...
export async function getStatsAPI() {
  const response = await axiosInstance.get('https://app.muon.net/stats/data.json');
  return response.data;
}
...
export async function getMigrationDataAPI({walletAddress}: {walletAddress: string}) {
