<script setup lang="ts">
  ...
  chatStore.reloadChats().then(() => {
    for (let chat_index in chatStore.chats) {
      const chat = chatStore.chats[chat_index];
...
<template>
  <div
    ...
    class="p-4 bg-gray-100 w-64 flex flex-col text-gray-500"
    ...
    <ul class="overflow-auto">
      <li class="mb-2">
        <a
          ...
          @click="chatStore.setCurrentChatId(null)"
        >
          New chat
        </a>
      </li>
      ...
      >
        <div class="truncate">
          <a
            ...
            @click="chatStore.setCurrentChatId(chat.id ? chat.id : null)"
          >
            {{ chat.title }}
          </a>
        </div>
