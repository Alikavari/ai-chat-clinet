...
import {type W3bNumber} from '../types/wagmi';
...
export function w3bNumberFromBigint(bigint: bigint, decimals = 18): W3bNumber {
  return {
    ...
    dsp:
      Math.floor(
        Number(ethers.formatEther(bigint * BigInt(10 ** 18 / 10 ** decimals))) * 100
      ) / 100,
    big: bigint,
...
export function w3bNumberFromString(amount: string, decimals = 18): W3bNumber {
  if (!amount) return {dsp: 0, hStr: '', big: BigInt(0), bigStr: BigInt(0).toString()};
